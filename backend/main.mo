...
// Persistent actor that carries the customDomain field across upgrades
...
actor {
  ...
  public shared ({ caller }) func setCustomDomain(domain : Text) : async () {
    if (not (AccessControl.isAdmin(accessControlState, caller))) {
      Runtime.trap("Unauthorized: Only admin can set domain");
    };
    customDomain := ?domain;
  };
